...
#
# The v1 debug patch paired </think> with <response_phase> in the same PEG that
# also generated the eager grammar. That made the model unable to emit its
# native </think> close unless the complete paired sequence matched, which can
# trap Qwen in very long reasoning. Runtime parsing still benefits from the
# paired boundary to avoid literal </think> collisions, but generation must
# keep the native reasoning close and require phase only *after* it.
# ---------------------------------------------------------------------------
...
'''        // Content only parser
    ...
    // The parser used on streamed/generated text is collision-hardened and
    // tolerant. The grammar below is rebuilt in strict mode for fresh Responses
    // generations so phase is mandatory without changing native </think>.
...
# Log the request-level effort, the effort marker actually rendered into the
# Qwen prompt, and the independent reasoning token budget. This intentionally
# stays in the debug branch only.
...
''',
'''    // Reasoning budget: pass parameters through to sampling layer.
    // reasoning_effort is a template-level instruction; this budget is an
    // independent hard/soft control and normally remains -1 (unlimited).
    {
        int reasoning_budget = json_value(body, "reasoning_budget_tokens",
                               json_value(body, "thinking_budget_tokens", -1));
        if (reasoning_budget == -1) {
...
# ---------------------------------------------------------------------------
# Regression coverage for the split architecture and Responses effort bridge.
# ---------------------------------------------------------------------------
...
'''    const auto phase_params = common_chat_templates_apply(tmpls.get(), phase_inputs);
    ...
    // Codex Responses reasoning.effort must survive the wire conversion and be
    // rendered into the actual Qwen model prompt.
...
# The v1 regression expected strict parsing itself. With split parsing, strictness
# belongs to the generation grammar while runtime parsing remains collision-safe.
test = replace_once(
    test,
...
'''    // Runtime parsing remains tolerant/collision-safe. Fresh generation is
    // strict through phase_params.grammar; without a phase, no visible content
    // may escape from this parser even if the input is malformed/legacy.
