...
# Keep phase structurally separate from reasoning. Under the Responses-only
# protocol it is a required internal block immediately after the optional
# <think> block and before visible content/tool calls.
replace_once(
    "common/chat.cpp",
...
# Update focused regressions to assert that the phase is a separate internal
# block after reasoning and is held back atomically during partial streaming.
replace_once(
...
# Thinking-disabled Responses still get a phase block: the Jinja generation
# prompt supplies an empty think block, then the model starts with phase.
replace_once(
    "tests/test-qwen38-codex-template.cpp",
